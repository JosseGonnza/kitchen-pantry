// Kitchen Pantry — Vanilla UI
// Endpoints:
// - POST /api/products
// - GET  /api/products
// - POST /api/products/{productName}/batches
// - GET  /api/products/{productName}/batches
// - POST /api/products/{productName}/consume
// - GET  /api/stock/summary
// - GET  /api/stock/low?threshold=5
// - GET  /api/stock/low?category=GRAINS&threshold=5
...
function errorText({ res, body }) {
  const msg = body?.message ? String(body.message) : "Request failed";
  return `(${res.status}) ${msg}`;
}

// ---- Low stock state ----
let lowFilter = { threshold: null, category: null }; // null = disabled
let lowSet = new Set(); // productName strings

function setLowAppliedTag() {
  if (lowFilter.threshold === null) {
    el.lowApplied.textContent = "none";
    return;
  }
  const c = lowFilter.category ? lowFilter.category : "All";
...
function boxesHtml(qty) {
  ...
  if (qty === 0) html += `<span class="badge">empty</span>`;
  return html;
}
...
function productBadges(summary) {
  ...
  const parts = [];
  parts.push(`<span class="badge">${batches} batch${batches === 1 ? "" : "es"}</span>`);
  parts.push(`<span class="badge">next ${escapeHtml(next)}</span>`);
  if (lowSet.has(summary.productName)) {
    parts.push(`<span class="badge low">low stock</span>`);
  }
  return parts.join("");
}
...
function renderShelves(summary) {
  if (!summary.length) {
    el.shelves.innerHTML = `<p class="empty">No products yet. Add some.</p>`;
    return;
  }

  const grouped = groupByCategory(summary);

  // sort categories and products for stable UI
    ...
    for (const p of items) {
      ...
      productsHtml += `
        <div class="product-row ${low ? "low" : ""}">
          ...
          <div class="actions">
            <button class="iconbtn" data-act="fill-batch" data-name="${escapeHtml(p.productName)}">+ Batch</button>
            <button class="iconbtn" data-act="fill-consume" data-name="${escapeHtml(p.productName)}">Consume</button>
            <button class="iconbtn" data-act="view-batches" data-name="${escapeHtml(p.productName)}">View</button>
          </div>
        </div>
      `;
    }
    ...
    html += `
      <div class="shelf">
        <div class="shelf-head">
          ...
          <div class="shelf-meta">${items.length} products</div>
        </div>
  ...
  el.shelves.querySelectorAll("[data-act]").forEach((btn) => {
    btn.addEventListener("click", async () => {
      ...
      if (act === "fill-batch") {
        ...
        setMsg(el.batchMsg, `Selected "${name}"`, "ok");
        return;
      }
      ...
      if (act === "fill-consume") {
        ...
        setMsg(el.consumeMsg, `Selected "${name}"`, "ok");
        return;
      }
...
async function refreshAll() {
  ...
  el.shelves.innerHTML = `<p class="empty">Loading…</p>`;

  try {
    await loadLowStock();
    const summary = await loadSummary();
    renderShelves(summary);
    el.lastUpdated.textContent = `updated ${nowTag()}`;
  } catch (e) {
    el.shelves.innerHTML = `<p class="empty">Failed to load shelves: ${escapeHtml(e.message)}</p>`;
  }
}
...
async function openBatchesModal(productName) {
  el.modalTitle.textContent = `Batches · ${productName}`;
  el.modalSubtitle.textContent = "Ordered by expiry (soonest first)";
  el.modalBody.innerHTML = `<p class="empty">Loading…</p>`;
  showModal();

  const { ok, res, body } = await request(API.batches(productName));
  if (!ok || !Array.isArray(body)) {
    el.modalBody.innerHTML = `<p class="empty">Failed: ${escapeHtml(errorText({ res, body }))}</p>`;
    return;
  }

  const batches = sortBatchesByExpiry(body);
  if (!batches.length) {
    el.modalBody.innerHTML = `<p class="empty">No batches for this product yet.</p>`;
    return;
  }
    ...
    return `
      <div class="batch-item">
        <div class="left">
          ...
          <div class="date">Expiry: ${escapeHtml(b.expiryDate)}</div>
        </div>
        <div class="right">
...
el.addProductForm.addEventListener("submit", async (e) => {
  e.preventDefault();
  setMsg(el.productMsg, "Creating…");

  const name = el.productName.value.trim();
  const category = el.productCategory.value;

  if (!name) {
    setMsg(el.productMsg, "Name is required", "err");
    return;
  }
  ...
  setMsg(el.productMsg, "Created ✅", "ok");
  el.productName.value = "";
  el.batchProductName.value = name; // convenience
  await refreshAll();
});

el.addBatchForm.addEventListener("submit", async (e) => {
  e.preventDefault();
  setMsg(el.batchMsg, "Adding…");

  const productName = el.batchProductName.value.trim();
  const amount = toInt(el.batchAmount.value);
  const expiry = el.batchExpiry.value; // must be YYYY-MM-DD

  if (!productName) {
    setMsg(el.batchMsg, "Product name is required", "err");
    return;
  }
  if (!expiry) {
    setMsg(el.batchMsg, "Expiry date is required", "err");
    return;
  }
  if (!Number.isFinite(amount) || amount <= 0) {
    setMsg(el.batchMsg, "Amount must be greater than zero", "err");
    return;
  }
  ...
  setMsg(el.batchMsg, "Added ✅", "ok");
  await refreshAll();
});

el.consumeForm.addEventListener("submit", async (e) => {
  e.preventDefault();
  setMsg(el.consumeMsg, "Consuming…");

  const productName = el.consumeProductName.value.trim();
  const amount = toInt(el.consumeAmount.value);

  if (!productName) {
    setMsg(el.consumeMsg, "Product name is required", "err");
    return;
  }
  if (!Number.isFinite(amount) || amount <= 0) {
    setMsg(el.consumeMsg, "Amount must be greater than zero", "err");
    return;
  }
  ...
  setMsg(el.consumeMsg, "Consumed ✅", "ok");
  await refreshAll();
});
...
el.lowStockForm.addEventListener("submit", async (e) => {
  ...
  if (!Number.isFinite(threshold) || threshold < 0) {
    setMsg(el.lowMsg, "Threshold must be a non-negative number", "err");
    return;
  }
...
(function init() {
  setLowAppliedTag();
  el.lastUpdated.textContent = "updated —";
  refreshAll();
})();
